...
class BaseTestCase(TestCase):
    ...
    def create_song(self, album_instance, artist_instance, **kwargs):
        data = {
            ...
            "song_file": File(
                open(
                    "./media/tests/BIG SWIMMER - Racks (Official Music Video).mp3", "rb"
                )
            ),
            **kwargs,
        }
...
class TestSerializerLogic(BaseTestCase):
    def test_serializers(self):
        ...
        song_instance = self.create_song(
            album_instance=album_instance, artist_instance=artist_instance
        )

        user_albums = Album.objects.filter(user=self.user).prefetch_related(
            "label", "songs__artists"
        )
...
class TestUserAlbumsAPIView(BaseTestCase, APITestCase):
    ...
    def serialize_user_albums(self, request: Request):
        ...
        serialized_data = AlbumNestedSerializer(
            user_albums, many=True, context={"request": request}
        ).data
        ...
        for album_data in serialized_data:
            ...
            for song_data in album_data["songs"]:
                if "song_file" in song_data and song_data["song_file"]:
                    song_data["song_file"] = request.build_absolute_uri(
                        song_data["song_file"]
                    )
